...
namespace XPWLibrary.Controllers
{
    public class InvoiceControllers
    {
        ...
        public List<InvoiceBodyData> GetInvoiceBody(InvoiceData ob)
        {
            ...
            string sql = $"SELECT distinct * FROM TBT_ISSUEDETAIL where issuingkey = '{ob.RefInv}' AND ORDERQTY > 0 ORDER BY PONO,KIDS,SIZES,LOTNO,SEQ ,CTN";
            if (StaticFunctionData.Factory == "INJ")
            {
                sql = $"SELECT distinct * FROM TBT_ISSUEDETAIL where issuingkey = '{ob.RefInv}' AND ORDERQTY > 0 ORDER BY PARTNO,PONO,CTN";
            }
            Console.WriteLine(sql);
            DataSet dr = new ConnDB().GetFill(sql);
            int i = 0;
